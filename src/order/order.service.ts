...
export class OrderService {
  ...
  async create(createOrderDto: CreateOrderDto): Promise<Order> {
    try {
      const { userId, products, price, status } = createOrderDto; // productIds instead of productIds
      ...
      const order = this.orderRepository.create({
        user, // Associate the order with the user
        products: cartItems, // cartItems is an array of CartItem[]
        price,
        status,
      });
  ...
  async update(id: string, updateOrderDto: UpdateOrderDto): Promise<Order> {
    try {
      const { userId, products, price, status } = updateOrderDto; // productIds instead of productIds
      ...
      if (products) {
        const cartItems = await this.cartItemRepository.find({
          where: { id: In(products) }, // Fetch CartItems by their IDs
          relations: ['product'], // Get related products
        });
